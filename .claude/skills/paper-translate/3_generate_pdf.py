...
用法: python3 3_generate_pdf.py <markdown_path> [--font <ttf_path>] [--clean]
  --clean  清除数学公式缓存，强制重新渲染所有公式
"""
import sys, os, re, hashlib, io, shutil
import markdown
...
def markdown_to_pdf(md_path, pdf_path=None, font_path=None):
    ...
    # 移除块级公式后的 <br> 标签（会导致 KaTeX 解析错误）
    content = re.sub(r'\$\$(.+?)\$\$<br\s*/?>', r'$$\1$$', content, flags=re.DOTALL)
...
p img[src*="math"]:only-child{{display:block;height:auto;margin:8px auto;}}
...
if __name__ == '__main__':
    if len(sys.argv) < 2:
        print("用法: python3 3_generate_pdf.py <markdown_path> [--font <ttf_path>] [--clean]")
        print("示例: python3 3_generate_pdf.py paper_zh.md")
        print("      python3 3_generate_pdf.py paper_zh.md --clean  # 清除公式缓存")
        sys.exit(1)
